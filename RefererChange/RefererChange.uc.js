...
refererChanger.init = function () {
    ...
    var menuitem = document.createElement('menuitem');
		...
		menuitem.setAttribute('onclick', 'if (event.button == 2) {event.preventDefault(); refererChanger.edit();}else if(event.button == 1) { event.preventDefault(); refererChanger.RCToggle();}');
...
refererChanger.alert = function(aString, aTitle) {
    Cc['@mozilla.org/alerts-service;1'].getService(Ci.nsIAlertsService)
        .showAlertNotification("", aTitle || "RefererChanger" , aString, false, "", null);
};
