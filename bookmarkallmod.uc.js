...
(function() {
	...
	function bookmarkall() {
		...
		if (k == undefined) {
			...
			for (i = 1 + k; i < pref.dirs.length; i++) {
				target_folder = bookmarksService.createFolder(target_folder, pref.dirs[i], 0);
			}
		}
		var duoyu = new Array();
	...
	function savekey() {
		var r = /^(?:alt|ctrl|meta|shift)$/i,
			keys = pref.savekey.split('+').map(function(t) {
				return +(t = t.trim()) || (t.length < 3 ? t.toUpperCase().charCodeAt() : r.test(t) ? t.toLowerCase() + 'Key' : 0)
			}).filter(function(t) {
				return t
			});
	...
	function switchkey() {
		var r = /^(?:alt|ctrl|meta|shift)$/i,
			keys = pref.switchkey.split('+').map(function(t) {
				return +(t = t.trim()) || (t.length < 3 ? t.toUpperCase().charCodeAt() : r.test(t) ? t.toLowerCase() + 'Key' : 0)
			}).filter(function(t) {
				return t
			});
